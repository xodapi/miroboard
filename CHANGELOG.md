...
### Fixed / Исправлено
- Нотации: оборванная связь больше не становится узлом, удаление конца снимает ребро, копия карты сохраняет родителя, роль не теряется при вставке, недопустимая связь не записывается. Упоминание AML в документе .mboard не отвергает файл
### Added / Добавлено
- Нотации eEPC, VACD и интеллект-карта на том же графе и в схеме v1: палитра «Нотации», метка в `profileData`, пустая метка не пишется. Экспорт ARIS AML не импортируется
- Подпись на стрелке и линии: двойной клик по штриху, перетаскивание относительно середины маршрута. Нулевой сдвиг не пишется; схема остаётся v1
- Свободная стрелка и линия прилипают к фигуре и следуют за ней, в том числе после сохранения. Это не поток BPMN и не ребро: `content.link` необязателен, схема остаётся v1, удаление фигуры оставляет стрелку
