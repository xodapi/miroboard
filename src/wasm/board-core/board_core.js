 ...
 * @returns {number}
 */
export function clamp_scale(value) {
    const ret = wasm.clamp_scale(value);
    return ret;
}
 ...
 * @returns {number}
 */
export function snap_to_grid(value, grid_size) {
    const ret = wasm.snap_to_grid(value, grid_size);
    return ret;
}
 ...
 * @returns {string}
 */
export function validate_bpmn(model_json) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ptr0 = passStringToWasm0(model_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.validate_bpmn(ptr0, len0);
        deferred2_0 = ret[0];
        deferred2_1 = ret[1];
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('board_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
