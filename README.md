...
Команда `▶ Запуск` запускает первый детерминированный token runner. Он
подсвечивает путь токена от стартового до конечного события. Для XOR/OR пока
выбирается первый sequence flow, а параллельные шлюзы будут добавлены в этапе
полной симуляции.
